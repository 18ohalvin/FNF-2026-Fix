...
/**
 * Fetch live slot capacities & availability
 */
export async function apiGetSlotCapacities() {
  try {
    return await fetchWithApiFallback('/api/slots/capacity')
  } catch (err) {
    console.warn('[API Client] Failed to fetch slot capacities:', err)
    return { success: false, maxCapacity: 25, capacities: {} }
  }
}
...
export async function apiFetchOccupancy(dayStr = '') {
  try {
    ...
    return {
      ...
      checkedInToday: 0,
      checkedOutToday: 0,
      eventDayText: dayStr || 'DAY 1 - SATURDAY, 19 SEPTEMBER 2026',
      recentScans: []
    }
  }
}
